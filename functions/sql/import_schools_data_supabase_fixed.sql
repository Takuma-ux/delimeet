-- Supabase用 学校データインポートSQL（修正版）
-- 注意: このスクリプトはSupabaseのSQL Editorで実行してください
-- 事前に create_temp_import_table.sql を実行して temp_schools_import テーブルを作成してください
...
-- インポート結果確認
SELECT COUNT(*) as imported_schools FROM schools;
SELECT COUNT(*) as source_rows FROM temp_schools_import;
...
ON CONFLICT DO NOTHING;  -- 重複は無視

-- 略称データ確認
SELECT COUNT(*) as alias_count FROM school_aliases;
...
SELECT 
    school_type,
    COUNT(*) as count,
    ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM schools), 2) as percentage
FROM schools 

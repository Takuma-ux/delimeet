...
class _WebMapSearchPageState extends State<WebMapSearchPage> {
  ...
  List<js.JsObject> _groupMarkers = [];
  ...
  // グローバルなマーカークリックハンドラー
  static final Map<String, Restaurant> _markerRestaurants = {};
  ...
  void initState() {
    ...
    _initializeLocation();
    _loadGroups(); // グループ情報の読み込みを追加
  }
  
  void _setupGlobalCallback() {
    // グローバルコールバックは不要になったため、空の実装
    print('[WebMapSearch] グローバルコールバック設定（簡略化）');
  }
  ...
  void _createMapWithLegacyAPI(html.DivElement mapElement, js.JsObject mapOptions, js.JsObject googleMaps) {
    try {
      ...
      print('[WebMapSearch] エラースタック: ${e.toString()}');
      throw e;
    }
  }
  ...
  void _addRestaurantMarkers() {
    ...
    try {
      ...
      _markerRestaurants.clear(); // グローバルマップもクリア
      ...
      for (int i = 0; i < _restaurants.length && i < 50; i++) {
        ...
        if (restaurant.latitude != null && restaurant.longitude != null) {
          ...
          // マーカーIDを生成してレストラン情報を保存
          final markerId = 'marker_${restaurant.id}_$i';
          _markerRestaurants[markerId] = restaurant;
          ...
          try {
            ...
            print('[WebMapSearch] マーカー作成: $markerId for ${restaurant.name}');
            
            // より安全なクリックイベント設定
            final clickHandler = js.allowInterop((dynamic e) {
              print('[WebMapSearch] マーカークリック検出: $markerId');
            ...
            // Google Maps APIのイベントリスナーを設定
            try {
              final maps = js.context['google']['maps'];
              if (maps != null) {
                maps['event'].callMethod('addListener', [marker, 'click', clickHandler]);
                print('[WebMapSearch] イベントリスナー設定成功: $markerId');
              } else {
                print('[WebMapSearch] Google Maps APIが見つかりません');
              }
            } catch (e) {
              print('[WebMapSearch] イベントリスナー設定エラー: $e');
            }
          } catch (e, stack) {
            print('[WebMapSearch] マーカークリックイベント設定エラー: $e');
            print('[WebMapSearch] エラースタック: $stack');
          }
  ...
  Future<void> _loadGroups() async {
    try {
      // レストラン募集グループのみを取得
      final groups = await _groupService.getRestaurantMeetupGroups().first;
      if (mounted) {
        setState(() {
          _groups = groups;
        });
      ...
      print('[WebMapSearch] グループ読み込みエラー: $e');
    }
  }

  // グループマーカーを追加
  void _addGroupMarkers() {
    if (_map == null || _googleMaps == null) return;
    ...
    for (final group in _groups) {
      if (group.restaurantInfo != null && 
          group.restaurantInfo!['latitude'] != null && 
          group.restaurantInfo!['longitude'] != null) {
        try {
          final marker = js.JsObject(_googleMaps!['Marker']);
          
          marker.callMethod('setPosition', [js.JsObject.jsify({
            'lat': group.restaurantInfo!['latitude'],
            'lng': group.restaurantInfo!['longitude'],
          })]);
          
          marker.callMethod('setMap', [_map]);
          marker.callMethod('setTitle', [group.name]);
          
          // グループマーカー用のカスタムアイコン（ピンク色の円）
          marker.callMethod('setIcon', [js.JsObject.jsify({
            'path': js.context['google']['maps']['SymbolPath']['CIRCLE'],
            'scale': 10,
            'fillColor': '#E91E63',
          ...
          // クリックイベントを追加
          final clickHandler = js.allowInterop((dynamic e) {
            print('[WebMapSearch] グループマーカークリック: ${group.name}');
            _showGroupJoinDialog(group);
          });
          
          js.context['google']['maps']['event']
            .callMethod('addListener', [marker, 'click', clickHandler]);
          
          _groupMarkers.add(marker);
        } catch (e) {
          print('[WebMapSearch] グループマーカー作成エラー: $e');
        }
      }
    }
  }
  ...
  // グループ参加ダイアログを表示
  void _showGroupJoinDialog(Group group) {
                  ...
                  try {
                    ...
                    if (mounted) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text('グループへの参加に失敗しました: $e'),
  ...
  Widget _buildRestaurantCard(Restaurant restaurant) {
    return Card(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(12),
        onTap: () => _showRestaurantActionDialog(restaurant),
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Row(
            children: [
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      // 基本情報
                      if (restaurant.nearestStation?.isNotEmpty == true) ...[
                        _buildInfoRow(Icons.train, '最寄駅', restaurant.nearestStation!),
                        const SizedBox(height: 12),
                      ],
                      if (restaurant.priceRange?.isNotEmpty == true) ...[
                        _buildInfoRow(Icons.attach_money, '価格帯', restaurant.priceRange!),
                        const SizedBox(height: 12),
                      ],
                      if (restaurant.address?.isNotEmpty == true) ...[
                        _buildInfoRow(Icons.location_on, '住所', restaurant.address!),
                        const SizedBox(height: 12),
                      ],
                      if (restaurant.hotpepperUrl?.isNotEmpty == true) ...[
                        _buildInfoRow(Icons.link, 'ホットペッパー', '詳細ページあり'),
                        const SizedBox(height: 12),
                      ],
  ...
  void _showRestaurantActionDialog(Restaurant restaurant) {
    print('[WebMapSearch] レストランアクション表示開始: ${restaurant.name}');
    print('[WebMapSearch] コンテキスト確認: ${context != null}');
    print('[WebMapSearch] mounted確認: $mounted');
    print('[WebMapSearch] レストラン情報: ${restaurant.name}, ${restaurant.category}');
    
    try {
      print('[WebMapSearch] showModalBottomSheet呼び出し開始');
      showModalBottomSheet(
        context: context,
        isScrollControlled: true,
        backgroundColor: Colors.transparent,
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (restaurant.nearestStation?.isNotEmpty == true) ...[
                            ...
                            Row(
                              children: [
                                Icon(Icons.train, size: 14, color: Colors.grey.shade500),
                                const SizedBox(width: 4),
                                Text(
                                  restaurant.nearestStation!,
                                  style: TextStyle(
      ...
      print('[WebMapSearch] レストランアクション表示完了: ${restaurant.name}');
    } catch (e) {
      print('[WebMapSearch] レストランアクション表示エラー: $e');
    }
  }
  ...
  // グループ募集作成を修正
  void _createGroupMeetup(Restaurant restaurant) async {
    ...
    try {
      ...
      final groupId = await _groupService.createRestaurantGroup(
        ...
        eventDateTime: tomorrow.add(const Duration(hours: 19)), // 翌日19時
        eventEndDateTime: tomorrow.add(const Duration(hours: 21)), // 翌日21時
        minMembers: 2,
        maxMembers: 4,
      ...
      if (mounted) {
        ...
        // グループチャットページに遷移
                // グループ情報を取得してからチャットページに遷移
        final group = await _groupService.getGroup(groupId) ?? Group(
          id: groupId,
          name: groupName,
          description: 'レストラン「${restaurant.name}」での食事会',
          createdBy: _groupService.currentUserId!,
          members: [_groupService.currentUserId!],
          admins: [_groupService.currentUserId!],
